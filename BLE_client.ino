...
void setup() {
  ...
  Lionel* lnl = new Lionel("e20a39f4-73f5-4bc4-a12f-17d1ad07a961");
...
void loop() {
  ...
  if (connected) {
    byte test[5] = {
      0x00, 0x4d, 0x00, 0x00, 0xb3 
    };
    pRemoteCharacteristic->writeValue(test,sizeof(test));
//    String newValue = "77 0 0";
//    byte buffer[newValue.length() + 1];
//
//    newValue.getBytes(buffer, newValue.length() + 1);
//
//    for (int i = 0; i < newValue.length() + 1; i++)
//      Serial.println(buffer[i], HEX);
//    
//    // Set the characteristic's value to be the array of bytes that is actually a string.
//    pRemoteCharacteristic->writeValue(buffer, newValue.length() + 1);
  }else if(doScan){
